...
void Estimator::optimization()
{
    ...
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        ...
        // problem.AddParameterBlock(para_Pose[i], SIZE_POSE, pose_manifold); // 7 维位姿
        // problem.AddParameterBlock(para_SpeedBias[i], SIZE_SPEEDBIAS); // 9 维速度和偏差
    }
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        //problem.AddParameterBlock(para_Ex_Pose[i], SIZE_POSE, pose_manifold); // 7 维外参
