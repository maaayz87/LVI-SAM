...
public:
    ...
    ros::Publisher pubRegisteredViCloud;
    ...
    vector<pcl::PointCloud<pcl::PointXYZRGB>::Ptr> viCloudKeyFrames;
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr viCloudLast; // 视觉特征点云
    ...
    mapOptimization()
    {
        ...
        subVicloud = nh.subscribe<sensor_msgs::PointCloud2>(PROJECT_NAME + "/vins/depth/depth_feature_world", 5, &mapOptimization::viCloudHandler, this, ros::TransportHints().tcpNoDelay());
        ...
        pubRegisteredViCloud = nh.advertise<sensor_msgs::PointCloud2>(PROJECT_NAME + "/lidar/mapping/vicloud_registered", 1);
    ...
    void allocateMemory()
    {
        ...
        viCloudLast.reset(new pcl::PointCloud<pcl::PointXYZRGB>()); //视觉特征点云
    ...
    void viCloudHandler(const sensor_msgs::PointCloud2ConstPtr &msgIn)
    {
        pcl::fromROSMsg(*msgIn, *viCloudLast);
    }
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            //std::cout << "pointer" << cloudOut->points[i].intensity << std::endl;
        }
        return cloudOut;
    }
...
#pragma omp parallel for num_threads(numberOfCores)
        for (int i = 0; i < cloudSize; ++i)
        {
            ...
            //std::cout << "rgb" << cloudOut->points[i].rgb << std::endl;
        }
        return cloudOut;
    }
    ...
    bool saveViMapService()
    {
        ...
        {
            //cout<<"12345"<<endl;
            if (viCloudKeyFrames[i]->empty()) {
                cout << "Warning: Empty input cloud or pose for frame " << i << endl;
            }
            *globalMapCloud += *transformPointCloud(viCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
            //cout<<"54321"<<endl;
            cout << "\r" << std::flush << "Processing vimap cloud " << i << " of " << cloudKeyPoses6D->size() << " ...";
        }

        pcl::io::savePCDFileBinary(saveMapDirectory + "/viMap.pcd", *globalMapCloud);
        ...
        if (!globalPath.poses.empty())
        {
            ...
            if(pathFile.is_open())
            {
                ...
                {
                    pathFile << globalPath.poses[i].pose.position.x << " " << globalPath.poses[i].pose.position.y << " " << globalPath.poses[i].pose.position.z << std::endl;
                }
            }
            pathFile.close();
        }
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    void publishFrames()
    {
        ...
        // publish registered vi feture cloud
        if (pubRegisteredViCloud.getNumSubscribers() != 0)
        {
            pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloudOut(new pcl::PointCloud<pcl::PointXYZRGB>());
            PointTypePose thisPose6D = trans2PointTypePose(transformTobeMapped);
            *cloudOut += *transformPointCloud(viCloudLast, &thisPose6D);
            publishCloud(pubRegisteredViCloud, cloudOut, timeLaserInfoStamp, odometryFrame);
        }
        // publish path
